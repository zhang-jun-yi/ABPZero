...
namespace MT.Tests
{
    public abstract class MTTestBase : AbpIntegratedTestBase<MTTestModule>
    {
        ...
        protected void UsingDbContext(Action<MTDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MTDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MTDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MTDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MTDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MTDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MTDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MTDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MTDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MTDbContext, Task<T>> func)
        {
            T result;
